...
var InfoBox = {
   ...
   _boxClosing : false,
      ...
      for(var i = 0; i < this._settings.animation.length; i++) {
        switch(this._settings.animation[i]) {
          ...
          case 'bottom' :
            ...
            break;
          case 'left' :
            this._boxIn.animate({'margin-left': -10}, {'queue': false, 'duration': this._settings.animDuration, 'complete': $.proxy(this.closeBox, this)});
            break;
          case 'right' :
            this._boxIn.animate({'margin-left': 10}, {'queue': false, 'duration': this._settings.animDuration, 'complete': $.proxy(this.closeBox, this)});
            break;
    ...
    closeBox : function() {
        if(this._boxClosing) this._box.css('display', 'none');
    },
